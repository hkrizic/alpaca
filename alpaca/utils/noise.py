...
def auto_noise_boost_radius(
    ...
    """
    Automatically determine noise boost radius based on image separation.

    Parameters
    ----------
    xgrid : np.ndarray
        X coordinate grid (for pixel scale).
    x_images, y_images : np.ndarray
        Point source image positions.
    frac_min_sep : float
        Fraction of minimum separation to use as radius.
    min_npix, max_npix : float
        Minimum and maximum radius in pixels.

    Returns
    -------
    radius : float
        Boost radius in arcsec.
    """
...
def boost_noise_around_point_sources(
    ...
    """
    Boost noise around point source positions to reduce their influence.
    ...
    x_images, y_images : np.ndarray
        Point source positions (arcsec).
    radius : float, optional
        Boost radius in arcsec. If None, computed automatically.
    f_max : float
        Maximum boost factor.
    frac_min_sep, min_npix, max_npix : float
        Parameters for auto radius calculation.

    Returns
    -------
    boosted_noise : np.ndarray
        Modified noise map with boosted regions.
    """
